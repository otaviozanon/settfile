...
export default async function handler(
  ...
  try {
    ...
    if (!file) {
      console.error(
        "Tmpfiles: no file in request. Available fields:",
    ...
    console.log(
      `Tmpfiles: uploading "${file.originalFilename}" (${buffer.length} bytes)`,
    );
    ...
    // Optional expire parameter (60-172800 seconds, default 3600 = 1 hour)
    const expire = (fields as any).expire || 3600;

    const tmpForm = new FormData();
    tmpForm.append("file", blob, fileName);
    tmpForm.append("expire", expire.toString());
    ...
    if (!response.ok || !result.status) {
      console.error(
        `Tmpfiles: API returned ${response.status}:`,
    ...
    console.log(`Tmpfiles: upload success → ${result.data.url}`);
