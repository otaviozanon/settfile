...
export default async function handler(
  ...
  try {
    ...
    if (!file) {
      console.error(
        "Filebin: no file in request. Available fields:",
    ...
    console.log(
      `Filebin: uploading "${file.originalFilename}" (${fileBuffer.length} bytes)`,
    );
    ...
    if (response.status !== 201) {
      ...
      res.end(
        JSON.stringify({
          success: false,
          error: `Upload falhou no Filebin: HTTP ${response.status}`,
        }),
      );
      return;
    }
    ...
    console.log(`Filebin: upload success → ${fileUrl}`);
