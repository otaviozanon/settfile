...
export default async function handler(
  ...
  try {
    ...
    if (!file) {
      console.error(
        "Ufile: no file in request. Available fields:",
    ...
    console.log(
      `Ufile: uploading "${fileName}" (${fileData.byteLength} bytes)`,
    );

    // 1. Create session
    console.log("Ufile: creating session...");
    ...
    console.log(`Ufile: session created (fuid=${fuid})`);

    // 2. Upload chunk
    console.log("Ufile: uploading chunk...");
    ...
    if (!chunkResp.ok || !chunkText.includes("Uploaded successfully")) {
      console.error(
        `Ufile: chunk upload failed (${chunkResp.status}):`,
        chunkText,
      );
      res.statusCode = 500;
      res.setHeader("Content-Type", "application/json");
    ...
    console.log("Ufile: chunk uploaded");

    // 3. Finalize
    console.log("Ufile: finalizing...");
    ...
    console.log(`Ufile: upload success → ${finalJson.url}`);
